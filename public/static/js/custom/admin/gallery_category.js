try {
    // Define form element
    const addCategoryForm = document.getElementById('add_gallery_category_form');
...
// Submit button handler
    const CategorySubmitButton = document.getElementById('add_gallery_submit');
...
function editGalleryCategory(id) {
    ...
    blockUI.block();
    $.ajax({
        url: "/adb/edit-gallery-category/" + id,
        type: "GET",
        dataType: "json",
        success: function (data) {
            ...
            $("#edit_gallery_category").modal("show");
            blockUI.release();
        },
        error: function () {
            $("#edit_gallery_category").modal("hide");
            blockUI.release();
        }
    });
}
    ...
    function deleteGalleryCategory(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.replace("/adb/delete-gallery-category/" + id);
            }
        });
    }
...
try {
    // Define form element
    const editCategoryForm = document.getElementById('edit_gallery_form');
// Init form validation rules. For more info check the FormValidation plugin's official documentation:https://formvalidation.io/
...
// Submit button handler
    const EditCategorySubmitButton = document.getElementById('edit_gallery_submit');
